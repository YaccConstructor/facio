...
module Implementation = 
    ...
    let mutable shiftQuantity = 0
    let mutable reduceQuantity = 0
    let mutable gotoQuantity = 0
    ...
    let interpret (tables: Tables<'tok>) lexer (lexbuf : LexBuffer<_>) initialState =                                                                      
        ...
        while not finished do                                                                                    
            if stateStack.IsEmpty then 
                ...
                if kind = shiftFlag then ( //SHIFT
                ...
                ) elif kind = reduceFlag then //REDUCE
                    reduceQuantity <- reduceQuantity + 1
                    ...
                    try                                                                                               
                        ...
                        let currState = stateStack.Peep()
                        let newGotoState = gotoTable.Read(int tables.productionToNonTerminalTable.[prod], currState) //GOTO
                        stateStack.Push(newGotoState)
                        gotoQuantity <- gotoQuantity + 1
#if __DEBUG
        ...
        printfn "Shift: %d" shiftQuantity
        printfn "Reduce: %d" reduceQuantity
        printfn "Goto: %d" gotoQuantity                                                                                   
